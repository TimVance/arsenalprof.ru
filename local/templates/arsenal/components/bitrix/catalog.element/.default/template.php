...
<div class="goods__wrap bx-catalog-element" id="<?=$itemIds['ID']?>"
	itemscope itemtype="http://schema.org/Product">
    <div class="goods__box">
        <? if($arResult["PROPERTIES"]["KHIT"]["VALUE"] == 1) echo '<div class="hit">Хит</div>';?>
        <? if($arResult["PROPERTIES"]["NOVINKA"]["VALUE"] == 1) echo '<div class="new">Новинка</div>';?>
        <? if ($price['DISCOUNT'] > 0) echo '<div class="wholesale">-'.$price["PERCENT"].'%</div>'; ?>
    </div>
	<div class="container-fluid flexBetween">
        <div class="product-item-detail-slider-container" id="<?=$itemIds['BIG_SLIDER_ID']?>">
            ...
            {
                ?>
                <div class="product-item-detail-compare-container product-item-compare-container">
                    <div class="product-item-detail-compare">
                        <div class="checkbox">
                            <label id="<?=$itemIds['COMPARE_LINK']?>">
                                <input type="checkbox" data-entity="compare-checkbox">
                                <div title="Cравнение товаров" class="card-favorites"></div>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <a title="Избранное" href="javascript:void(0)"  class="wishbtn<? if ($arResult["ID"] == $arResult["delay"]) echo ' in_wishlist '; ?>"
               onclick="add2wish(
                       '<?=$arResult["ID"]?>',
                       '<?=$price["ID"]?>',
                       '<?=$price["PRICE"]?>',
            ...
            <div class="product-item-detail-slider-images-container" data-entity="images-container">
                <? if (!empty($arResult["DETAIL_PICTURE"]['SRC'])) : ?>
                    <a rel="group1" class="fancybox" href="<?=$arResult["DETAIL_PICTURE"]['SRC']?>"><img src="<?=$arResult["DETAIL_PICTURE"]['SRC']?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?>></a>
                <? else: ?>
                    <a rel="group1" class="fancybox" href="/local/templates/arsenal/components/bitrix/catalog.section/.default/images/no_photo.png"><img src="/local/templates/arsenal/components/bitrix/catalog.section/.default/images/no_photo.png" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?>></a>
                <? endif; ?>
                <?
                if (!empty($arResult["PROPERTIES"]["MORE_PHOTO"]["ARRAY"]))
                {
                    foreach ($arResult["PROPERTIES"]["MORE_PHOTO"]["ARRAY"] as $key => $photo)
                    {
                        ?>

                        <a rel="group1" class="fancybox" href="<?=$photo['SRC']?>"><img src="<?=$photo['SRC']?>" alt="<?=$alt?>" title="<?=$title?>"<?=($key == 0 ? ' itemprop="image"' : '')?>></a>
        ...
        <div class="goods__info">
            ...
            if (!empty($arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]))
                echo '<div class="artikul">Арт. '.$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"].'</div>';

            ?>
            <div class="product-item-detail-pay-block goods__price_wrap">
                    <?
                        ...
                        switch ($blockName)
                        {
                            ...
                            case 'price':
                                ...
                                <div class="product-item-detail-info-container">
                                    ...
                                    ?>
                                    <div class="product-item-detail-price-current goods-price" id="<?=$itemIds['PRICE_ID']?>">
                            ...
                            case 'buttons':
                                ...
                                <?
                                $inBasket = 0;
                                if (isset($arResult["arBasketItems"][$arResult["ID"]])) $inBasket = 1;
                                ?>
                                <div style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;" data-entity="main-button-container">
                                    <div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
                                        <?
                                        if ($showAddBtn)
                                        {
                                            ?>
                                            <div class="product-item-detail-info-container add-over-basket" data="<?=$arResult["ID"]?>">
                                                <?php $max_count = CCatalogProduct::GetByID($arResult["ID"]); ?>
                                                <div class="change-field-wrap">
                                                    <span class="minus">-</span>
                                                    <input class="change_count_input" min="1" max="<?=$max_count["QUANTITY"]?>" type="text" value="1">
                                                    <span class="plus">+</span>
                                                </div>
                                                <a class="<? if($inBasket == 1) echo ' in-basket'; ?> btn <?=$showButtonClassName?> product-item-detail-buy-button"
                                                    href="javascript:void(0);">
                                                    <svg>
                                                        <use xlink:href="#basket"></use>
                                                    </svg>
                                                    <? if($inBasket == 0) { ?> <span><?=$arParams['MESS_BTN_ADD_TO_BASKET']?></span><? } ?>
                                                    <? if($inBasket == 1) { ?> <span>Добавлено</span><? } ?>
                                                </a>
                                            </div>
                                            <?
                                        }
...
//                    }
            ...
            <div class="row">
                <div class="col-sm-8 col-md-9 tabs-wrap">
                    <div class="row" id="<?=$itemIds['TABS_ID']?>">
                        <div class="col-xs-12">
                            <div class="product-item-detail-tabs-container">
                                <ul class="product-item-detail-tabs-list tabs">
                                    <?

                                    if (!empty($arResult['DISPLAY_PROPERTIES']) || $arResult['SHOW_OFFERS_PROPS'])
                                    {
                                        ?>
                                        <li class="product-item-detail-tab tab" data-entity="tab" data-value="properties">
                                    ...
                                    {
                                        ?>
                                        <li class="product-item-detail-tab active tab" data-entity="tab" data-value="description">
                                            <a href="javascript:void(0);" class="product-item-detail-tab-link">
                                    ...
                                    {
                                        ?>
                                        <li class="product-item-detail-tab tab" data-entity="tab" data-value="comments">
                                            <a href="javascript:void(0);" class="product-item-detail-tab-link">
                    ...
                    <div class="row" id="<?=$itemIds['TAB_CONTAINERS_ID']?>">
                        <div class="col-xs-12">
                            ...
                            {
                                ?>
                                <div class="product-item-detail-tab-content tab_content" data-entity="tab-container" data-value="properties">
                                    <?
                                    if (!empty($arResult['DISPLAY_PROPERTIES']))
                                    {
                                        ?>
                                        <div class="product-item-detail-properties table">
                                            <?
                                            foreach ($arResult['DISPLAY_PROPERTIES'] as $property)
                                            {
                                                ?>
		...
		<ul class="product-item-detail-tabs-list">
			...
			if (!empty($arResult['DISPLAY_PROPERTIES']) || $arResult['SHOW_OFFERS_PROPS'])
			{
				...
				<li class="product-item-detail-tab" data-entity="tab" data-value="properties">
					<a href="javascript:void(0);" class="product-item-detail-tab-link tab">
						<span><?=$arParams['MESS_PROPERTIES_TAB']?></span>
					</a>
				</li>
				<?
			...
			if ($arParams['USE_COMMENTS'] === 'Y')
			{
				...
				<li class="product-item-detail-tab" data-entity="tab" data-value="comments">
					<a href="javascript:void(0);" class="product-item-detail-tab-link tag">
						<span><?=$arParams['MESS_COMMENTS_TAB']?></span>
					</a>
				</li>
				<?
