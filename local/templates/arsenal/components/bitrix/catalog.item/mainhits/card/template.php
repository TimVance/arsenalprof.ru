...
<div href="<?=$item['DETAIL_PAGE_URL']?>" class="card-img fotorama">
    <img src="<?=$item['PREVIEW_PICTURE_SECOND']['SRC']?>" alt="">
</div>
	...
	<? endif; ?>
		...
		<span class="product-item-image-original" id="<?=$itemIds['PICT']?>"
			style="background-image: url('<?=!empty($item['PREVIEW_PICTURE_SECOND']) ? $item['PREVIEW_PICTURE_SECOND']['SRC'] : $item['PREVIEW_PICTURE']['SRC']?>'); <?=($showSlider ? 'display: none;' : '')?>">
		</span>
		<?
...
<div class="card__box">
                       <div class="card__info">
                            <? if($item["PROPERTIES"]["KHIT"]["VALUE"] == 1) echo '<div class="card__info-item card__info-hit">Хит</div>';?>
                            <? if($item["PROPERTIES"]["NOVINKA"]["VALUE"] == 1) echo '<div class="card__info-item card__info-new">Новинка</div>';?>
                            <? if($price['PERCENT'] > 0) echo '<div class="card__info-item card__info-wholesale">-'.$price["PERCENT"].'%</div>'; ?>
                        </div>
	<div class="product-item-title">
		<? if ($itemHasDetailUrl): ?>
		<a class="card-title" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$productTitle?>">
		<? endif; ?>
		<?=strlen($productTitle) > 48 ? mb_substr($productTitle, 0, 48).'...' : $productTitle?>
		<? if ($itemHasDetailUrl): ?>
		</a>
		<? endif; ?>
	</div>
	...
	if (!empty($arParams['PRODUCT_BLOCKS_ORDER']))
	{
		foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
		{
			...
			{
				case 'price': ?>
					<div class="product-item-info-container product-item-price-container card__price" data-entity="price-block">

						<span class="product-item-price-current card__price-new" id="<?=$itemIds['PRICE']?>">
							<?
							if (!empty($price))
							{
				...
				case 'buttons':
					?>
					<div class="product-item-info-container product-item-hidden card__basket" data-entity="buttons-block">
						<?
						if (!$haveOffers)
						{
                            if ($actualItem['CAN_BUY'])
                            {
                                ...
                                ?>
                                <div class="product-item-button-container add-over-basket product-item-button-hits" data="<?=$item["ID"]?>">
                                    ...
                                    <a class="<? if($inBasket == 1) echo 'in-basket '; ?>card__basket-icon btn" href="javascript:void(0)" rel="nofollow">
                                        <? if($inBasket == 1) { ?>
            ...
            <div class="product-item-compare">
                <div class="checkbox">
                    <label id="<?=$itemIds['COMPARE_LINK']?>" title="Сравнение товаров">
                        <input <? if (!empty($_SESSION["CATALOG_COMPARE_LIST"][4]["ITEMS"][$item["ID"]])) echo 'checked '; ?>type="checkbox" data-entity="compare-checkbox">
