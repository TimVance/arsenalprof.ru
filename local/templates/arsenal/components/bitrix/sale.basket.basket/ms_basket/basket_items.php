...
if(isset($arResult["ITEMS"]["DelDelCanBuy"]) && !empty($arResult["ITEMS"]["DelDelCanBuy"]))
{   
    foreach($arResult["ITEMS"]["DelDelCanBuy"] as $arItem)
    {
        echo '<input type="hidden" name="DELAY_'.$arItem["ID"].'" value="Y" />';
    }    
}
?>
    ...
    <div class="div_table">
        ...
        <div class="div_table_body">
            <?if(!isset($arResult["ITEMS"]["AnDelCanBuy"]) || empty($arResult["ITEMS"]["AnDelCanBuy"])):?>
                <p><font class="errortext"><?=GetMessage('MS_EMPTY_BASKET_TEXT');?></font></p>
            <?endif;?>
                ...
                <div class="row">
                            ...
                            if(!empty($arItem["SKU_PRODUCT"]))
                            {
                                ...
                                {   
                                    ...
                                    <div class="detail_color js_detail_prop">
                                        ...
                                        <?foreach($arSkuData["VALUES"] as $arProp):
                                        $li_active = "";
                                        ...
                                        ?>
                                        <li data-xml="<?=$arProp["UF_XML_ID"]?>" data-entity="basket" data-offer="<?=$arProp["OFFERS_ID"]?>" class="<?=$li_active?>" data-id="<?=$arItem["ID"]?>" data-property="<?=$arSkuData["CODE"]?>" data-ajax="<?=$arParams["AJAX_ID"]?>">
                            ...
                            if(!empty($arItem["SKU_PRODUCT"]))
                            {
                                ...
                                {   
                                    ...
                                    <div class="detail_size js_detail_prop">
                                        ...
                                        if(isset($arProp["DISABLE"]))
                                        {
                                            $li_disable = "li-disable";    
                                        }
                                        

                                        ?>
                                        <li class="<?=$li_active?> <?=$li_disable?>" data-entity="basket" data-xml="<?=$arProp["UF_XML_ID"]?>" data-offer="<?=$arProp["OFFERS_ID"]?>" data-id="<?=$arItem["ID"]?>" data-property="<?=$arSkuData["CODE"]?>" data-ajax="<?=$arParams["AJAX_ID"]?>">
                    ...
                    <div class='col-sm-2'>
                        <div class="wrap_del_change">
                            <p class="cart-delete-item"><a title="<?=GetMessage("MS_BASKET_DELETE")?>" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" rel="nofollow"><?=GetMessage("MS_BASKET_DELETE")?></a></p>
                            <p class="cart-delete-item"><a title="<?=GetMessage("MS_BASKET_DO_DELAY")?>" href="<?=str_replace("#ID#", $arItem["PRODUCT_ID"], $arUrls["new_delay"])?>" onclick="BX.ajax.insertToNode('<?=str_replace("#ID#", $arItem["PRODUCT_ID"], $arUrls["new_delay"])?>&bxajaxid=<?=$bxajaxid?>', 'comp_<?=$bxajaxid;?>'); return false;"  rel="nofollow"><?=GetMessage("MS_BASKET_DO_DELAY")?></a></p>
                        </div>
                    </div>
                </div>
    ...
    <div class="block_basket_count_wrap">
        <div class="block_basket_count">
            <div class="row">
                ...
                <div class="col-sm-7 <?if ($arParams["HIDE_COUPON"] != "Y"):?>col-sm-push-3<?else:?>pull-right<?endif;?>">
                    <p class="basket_count"><?=GetMessage("MS_BASKET_ITOGO")?>: <span><?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?></span></p>
                </div>
                <?if ($arParams["HIDE_COUPON"] != "Y"):?>
