...
global $USER;
$AddMenuLinks = unserialize( Option::get( "sotbit.b2bshop", "ADD_MENU_LINKS", 'a:0:{}' ) );
$MenuShow = Option::get( "sotbit.b2bshop", "MENU_ALL", '-1' );
$ShowThird = Option::get( "sotbit.b2bshop", "MENU_ALL_THIRD_SHOW", '' );
...
{
	$nBrand = Option::get( "sotbit.b2bshop", "COUNT_BRAND_MENU", "10" );
...
if( !empty( $arParams["IBLOCK_BANNER"] ) )
{
	...
	while ( $arBanners = $rsBanners->Fetch() )
	{
		$ID = $arBanners["ID"];
			...
			elseif($arBanners["CATALOG_ELEMENT_ID"]["0"] && $arBanners["ADD_LEFT_TEXT"] !== "Y" && (isset( $arLink[$arBanners["CODE"]] ) || isset( $arTextLink[$arBanners["NAME"]] ))) {
                if( !isset( $arRes["BANNER"][$ID] ) )
                {
                    $arRes["BANNER"][$ID] = $arBanners;
                }
	...
	if( is_array( $arElementsBannerId ) )
	{
		...
		while ( $arItem = $rsItem->GetNext() )
		{
			...
			$arItem["ARTICLE"] = $arItem["PROPERTY_CML2_ARTICLE_VALUE"];
		...
		$resPrice = CIBlockPriceTools::GetCatalogPrices( $arParams["IBLOCK_CATALOG"], $typePrice );
		$arMinPrice = array();
						...
						if( $PrevProd != $arOffer['PROPERTY_' . $mxResult['SKU_PROPERTY_ID'] . '_VALUE'] )
						{
							...
							foreach( $ar_price as $Price )
							{
								if( !isset( $MinPrice ) || (isset( $Price['PRICE'] ) && $Price['PRICE'] < $MinPrice) )
								{
                                    ...
                                    $Price['DISCOUNT'] = $arDiscounts[0]['VALUE'];
		...
		{ 
			...
			if( $MinPrice['DISCOUNT_PRICE'] && $MinPrice['DISCOUNT_PRICE'] != $MinPrice['PRICE'] )
			{
                ...
                $allBannerCatalog[$ProductId]["MIN_PRICE"]["DISCOUNT"] = number_format($MinPrice['DISCOUNT'], 0, '', ' ');
			}
		}
		unset( $arMinPrice );
		
		/*
		 * foreach ( $rsOffer as $offer )
		 * {
		 * if (empty( $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["VALUE"] ) || $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["VALUE"] > $offer["MIN_PRICE"]["VALUE"])
		 * {
		 * $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["VALUE"] = $offer["MIN_PRICE"]["VALUE"];
		 * $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["PRINT_VALUE"] = $offer["MIN_PRICE"]["PRINT_VALUE"];
		 * }
		 * if (empty( $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["DISCOUNT_VALUE"] ) || $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["DISCOUNT_VALUE"] > $offer["MIN_PRICE"]["DISCOUNT_VALUE"])
		 * {
		 * $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["DISCOUNT_VALUE"] = $offer["MIN_PRICE"]["DISCOUNT_VALUE"];
		 * $allBannerCatalog[$offer["LINK_ELEMENT_ID"]]["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"] = $offer["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"];
		 * }
		 * }
		 * unset( $rsOffer );
		 */
...
if( isset( $AddMenuLinks ) && is_array( $AddMenuLinks ) && count( $AddMenuLinks ) > 0 )
{
	...
	//printr($arResult);
	//$arBanners['PROPERTY_BRICK_MODE_VALUE']
