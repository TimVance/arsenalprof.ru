...
$modulePageResultCount = IntVal(COption::GetOptionString("arturgolubev.smartsearch", "search_max_count"));
if($modulePageResultCount) $arParams["PAGE_RESULT_COUNT"] = $modulePageResultCount;
...
if($this->InitComponentTemplate($templatePage))
{
	...
	if(strlen($folderPath) > 0)
	{
		...
		if (!$obSearch->selectedRowsCount() || $arResult["MODULE_SETTING"]["EXT_STANDART_SEARCH"] == 'Y') {
			$arResult["DEBUG"]["TYPE"] = "extended";
			
			$arResult["SEARCH"] = array();
			
			$arSmartParams = array();
			$arSmartParams["query_words"] = explode(' ', strtoupper($obSearch->Query->m_query));
