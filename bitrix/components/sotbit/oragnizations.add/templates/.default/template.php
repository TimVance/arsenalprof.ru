	...
	<div class="wrap_content">
		<div class="row">
			<div class="col-sm-22 col-md-20 col-lg-16 sm-padding-right-no">
				...
				{
					?>
					<div class="confirm-buyer"><?=Loc::getMessage('CONFIRM_BUYER')?></div>
					<?
				}
				else
				{
					...
					<form method="post" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data" id="add-org">
						...
						<div class="row">
							<div class="col-sm-10 col-md-8 sm-padding-right-no">
								<label><?=Loc::getMessage('SALE_PERS_TYPE')?></label>
							</div>
							<div class="col-sm-14 col-md-16">
								<select name="PERSON_TYPE" id="person-type">
									...
									{
										?>
									<option <?=($id==$arResult['PERSON_TYPE']['ID']?'selected':'')?> value="<?=$id?>"><?=$name?> / ИП</option>
										<?php
									}
									?>
								</select>
						...
						<div class="row">
							...
							<div class="col-sm-8">
								<input type="submit" class="btn-reset"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
							</div>
						</div>
					</form>
				<?}?>
