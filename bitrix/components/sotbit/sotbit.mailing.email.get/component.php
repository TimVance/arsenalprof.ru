...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	if ($arParams["TYPE"] == 'SECTION_ID')
	{
		...
		$arParams["PARAM_2"] = $arParams["PARAM_2:SECTION_ID"];
		$arParams["PARAM_3"] = $arParams["PARAM_3:SECTION_ID"];
	...
	{
		$arParams["PARAM_1"] = $arParams["TYPE"];
		$arParams["PARAM_2"] = $arParams["PARAM_2:PROPERTY"];
		$arParams["PARAM_3"] = $arParams["PARAM_3:PROPERTY"];
	...
	if ($USER->IsAuthorized () && empty ( $_COOKIE['MAILING_USER_MAIL_GET'] ))
	{
		...
		$answer = CSotbitMailingSubscribers::checkSubscribers ( array (
				'EMAIL_TO' => $EMAIL_USER 
		) );
		// ���� ��� ��������
		if ($answer)
	...
	if ($USER->IsAuthorized ())
	{
		...
		$dbSubscrib = CSotbitMailingSubscribers::GetList ( Array (), Array (
				'USER_ID' => $USER_ID 
		), false, array () );
		if ($resSubscrib = $dbSubscrib->Fetch ())
		{
			$arResult['SUBSCRIBER_INFO'] = $resSubscrib;
			$arResult['SUBSCRIBER_INFO']['CATEGORIES_ID'] = CSotbitMailingSubscribers::GetCategoriesBySubscribers ( $resSubscrib['ID'] );
	...
	// ������� ��� ���������
	$dbCategories = CSotbitMailingCategories::GetList ( Array (
			'ID' => "ASC" 
	), Array (
			'ACTIVE' => 'Y' 
	), false, array () );
