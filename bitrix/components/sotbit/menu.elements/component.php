...
if($this->StartResultCache())
{
	...
	else
	{
        ...
        $rsElements = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter), false, false, array("ID", "NAME", "DETAIL_PAGE_URL"));
		...
		while($arSection = $rsElements->GetNext())
		{   //printr($arSection);
			$arResult["SECTIONS"][] = array(
				"ID" => $arSection["ID"],
				"DEPTH_LEVEL" => 1,
				"~NAME" => $arSection["~NAME"],
				"~DETAIL_PAGE_URL" => $arSection["~DETAIL_PAGE_URL"],
			);
...
foreach($arResult["SECTIONS"] as $arSection)
{
	...
	$arResult["ELEMENT_LINKS"][$arSection["ID"]][] = urldecode($arSection["~DETAIL_PAGE_URL"]);
	$aMenuLinksNew[$menuIndex++] = array(
		htmlspecialcharsbx($arSection["~NAME"]),
		$arSection["~DETAIL_PAGE_URL"],
