...
$paramTemplate = CComponentUtil::GetTemplateProps('sotbit:sotbit.mailing.logic',  $arParams['MAILING_TEMPLATE'], "", $arParams);   
...
//получим данные по типу DATE_PERIOD_AGO
foreach($param_date_age_code as $code){
    if(isset($arParams[$code.'_from']) && isset($arParams[$code.'_to']) && isset($arParams[$code.'_type'])){
        ...
        $arrDateAgoNowParam = CSotbitMailingHelp::GetDateAgoNow($GetDateAgoNowParam);   
...
//если рассылка не была запущена поставим метку
$mailingInfo = CSotbitMailingEvent::GetByID($arParams['MAILING_EVENT_ID']);
if($mailingInfo['MAILING_WORK'] == 'N'){
    
    CSotbitMailingEvent::Update($mailingInfo['ID'], array(
        "MAILING_WORK" => 'Y',
        "MAILING_WORK_PARAM" => serialize($arParams)
    ));    
            
} 
//если рассылка уже работает
