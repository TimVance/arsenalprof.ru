...
//START
    ...
    foreach($Novelty_property as $key_Novelty_property=>$value_Novelty_property) {
        ...
        if(count($arrNoveltyProduct) >= $arParams['TOP_COUNT_FILLTER_NOVELTY_GOODS_FROM']) {
            ...
            $NOVELTY_GOODS_PRODUCT[$key_Novelty_property.$value_Novelty_property] = CSotbitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_TOP'] , array('PROPERTY_VALUE'=>$value_Novelty_property));         
            foreach($arrNoveltyProduct as $ItemNoveltyProduct) {
                ...
                $NOVELTY_GOODS_PRODUCT[$key_Novelty_property.$value_Novelty_property] .= CSotbitMailingHelp::ReplaceVariables($arParams["TEMP_NOVELTY_GOODS_LIST"] , $ItemNoveltyProduct);   
                $i++;           
            }
            $NOVELTY_GOODS_PRODUCT[$key_Novelty_property.$value_Novelty_property] .= CSotbitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_BOTTOM'] , array('PROPERTY_VALUE'=>$value_Novelty_property));       
        }    
    ...
    $arrEmailSend = array(); 
    $dbSubscribers = CSotbitMailingSubscribers::GetList(array(),array('CATEGORIES_ID' => $mailing_category_id ,'ACTIVE'=>'Y'), false, array('ID','USER_ID','EMAIL_TO'));
