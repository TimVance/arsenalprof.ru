...
//START
    ...
    // ������� ������� ������ � ���������
    // START
    $MailingCatSectionId = array();
    $FillterMailingRazdel = array();
    $categoriesLi = CSotbitMailingHelp::GetCategoriesInfo();
    $Novelty_section_id = array();
    foreach($categoriesLi as $v) {
        
        if(empty($arParams['CATEGORIES_ID'])) {      
        
            if($v['PARAM_1']=='SECTION_ID') {
                $FillterMailingRazdel[] = $v['ID'];  
                $Novelty_section_id[] = $v['PARAM_2'];  
                 
                $MailingCatSectionId[$v['ID']] = $v['PARAM_2'];      
            }                      
        }
        else {
            
            if(in_array($v['ID'], $arParams['CATEGORIES_ID'])) {
                $FillterMailingRazdel[] = $v['ID'];  
                $Novelty_section_id[] = $v['PARAM_2'];
    ...
    // ������� ���������� � �������� ���������
    // START
    $Novelty_section_id_INFO = array();
    $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'),array('ID'=>$Novelty_section_id),false,array('ID','NAME','SECTION_PAGE_URL','DESCRIPTION'));
    while($arSect = $rsSect->GetNext(true,false))
    {   
    
        if(strpos($arSect['SECTION_PAGE_URL'],'?')) {
            $arSect['SECTION_PAGE_URL'] = $arParams['MAILING_SITE_URL'].$arSect['SECTION_PAGE_URL'].'&MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=sotbit_mailing_'.$arParams['MAILING_EVENT_ID'];        
        } else {
            $arSect['SECTION_PAGE_URL'] = $arParams['MAILING_SITE_URL'].$arSect['SECTION_PAGE_URL'].'?MAILING_MESSAGE=#MAILING_MESSAGE#&utm_source=newsletter&utm_medium=email&utm_campaign=sotbit_mailing_'.$arParams['MAILING_EVENT_ID'];          
        }     
    

        foreach($arSect as $ku => $vu){
            unset($arSect[$ku]);
            $arSect['SECTION_'.$ku]  = $vu;       
        }                    
        
        $Novelty_section_id_INFO[$arSect['SECTION_ID']] = $arSect;
    }    
    // END
    ...
    foreach($Novelty_section_id as $section_id) {
    
        $arrNoveltyProduct = array();        
        
        // ������� ������ �������
        if($Novelty_section_id) {
            $arFilterNovelty['SECTION_ID'] = $section_id;  
            $arFilterNovelty['INCLUDE_SUBSECTIONS'] = 'Y';  
        } 
        
        include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/sotbit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_NOVELTY_GOODS_FILLTER_BEFORE.php");        
        ...
        {      
            ...
            include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/sotbit.mailing/php_files_mailing/".$arParams['MAILING_EVENT_ID']."/PHP_NOVELTY_GOODS_FILLTER_WHILE_AFTER.php"); 
        ...
        if(count($arrNoveltyProduct) >= $arParams['TOP_COUNT_FILLTER_NOVELTY_GOODS_FROM']) {
            $i = 0;  
            $NOVELTY_PRODUCT_ID_ARRAY[$section_id] = array();     
            $NOVELTY_GOODS_PRODUCT[$section_id] = CSotbitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_TOP'] , $Novelty_section_id_INFO[$section_id]);         
            foreach($arrNoveltyProduct as $ItemNoveltyProduct) {
                $ItemNoveltyProduct['BORDER_TABLE_STYLE'] = '';
                $NOVELTY_PRODUCT_ID_ARRAY[$section_id][] = $ItemNoveltyProduct['ID'];
                if($i > 0){
                    $ItemNoveltyProduct['BORDER_TABLE_STYLE'] = ' border-top: 1px solid #E6EAEC; ';  
                }             
                $NOVELTY_GOODS_PRODUCT[$section_id] .= CSotbitMailingHelp::ReplaceVariables($arParams["TEMP_NOVELTY_GOODS_LIST"] , $ItemNoveltyProduct);   
                $i++;           
            }
            $NOVELTY_GOODS_PRODUCT[$section_id] .= CSotbitMailingHelp::ReplaceVariables($arParams['TEMP_NOVELTY_GOODS_BOTTOM'] , $Novelty_section_id_INFO[$section_id]);       
        }    
    ...
    // ��������� ��������  ������� ������ � ���������� ���������
    // START
    $arr_subscr_id_category = CSotbitMailingSubscribers::GetCategoryByAllSubscribers();
    
    $arrEmailSend = array(); 
    $dbSubscribers = CSotbitMailingSubscribers::GetList(array(),array('CATEGORIES_ID'=>$FillterMailingRazdel,'ACTIVE'=>'Y'), false, array('ID','USER_ID','EMAIL_TO'));
    while($resSubscribers = $dbSubscribers->Fetch()){
        ...
        foreach($MailingCatSectionId as $categor=>$sectionId){
            if($arr_subscr_id_category[$resSubscribers['ID']][$categor]){
                $EmailSend['NOVELTY_GOODS'] .= $NOVELTY_GOODS_PRODUCT[$sectionId];    
           
                if($NOVELTY_PRODUCT_ID_ARRAY[$sectionId]){
                    $EmailSend['PARAM_MESSEGE']['NOVELTY_PRODUCT_PRODUCT_ID'] =  array_merge($EmailSend['PARAM_MESSEGE']['NOVELTY_PRODUCT_PRODUCT_ID'], $NOVELTY_PRODUCT_ID_ARRAY[$sectionId]);    
                }             
