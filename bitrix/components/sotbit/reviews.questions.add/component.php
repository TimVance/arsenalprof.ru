...
if(isset($CacheValues['SOTBIT_REVIEWS_QUESTIONS_ADD']))
{
	$arResult = $CacheValues['SOTBIT_REVIEWS_QUESTIONS_ADD'];
}

if(!isset($arResult) || !is_array($arResult) || sizeof($arResult)==0)
{
    
    $arResult['RECAPTCHA2_SITE_KEY'] = COption::GetOptionString(
            CSotbitReviews::iModuleID, 
            "QUESTIONS_RECAPTCHA2_SITE_KEY_" . SITE_ID, "");
    ...
    if ($arResult['BAN'] != "Y") {
        ...
        $arResult['REPEAT'] = COption::GetOptionString(
                CSotbitReviews::iModuleID, "QUESTIONS_REPEAT_" . SITE_ID, "-1");
        ...
        if ($arResult['REPEAT'] >= 0) {
            if ($USER->IsAuthorized()) {
                ...
                $IDx = COption::GetOptionString(CSotbitReviews::iModuleID, 
                        "QUESTIONS_ID_ELEMENT_" . SITE_ID, "ID_ELEMENT");
                if ($IDx == 'ID_ELEMENT')
                    $Filter = array(
                ...
                // find last
                $rsFields = QuestionsTable::getList(
                        array(
                                'select' => array(
                                        'ID',
                                        'DATE_CREATION'
                ...
                if ($Fields = $rsFields->Fetch()) {
                    ...
                    if (isset($Fields['ID']) && $Fields['ID'] > 0) {
                        if ($arResult['REPEAT'] == 0) {
                            ...
                            $LastTime = CIBlockFormatProperties::DateFormat(
                                    'd.m.Y H:i:s', 
                                    MakeTimeStamp($Fields['DATE_CREATION'], 
                                            CSite::GetDateFormat()));
                            
                            $ReadyTime = date('d.m.Y H:i:s', 
                                    strtotime(
                                            "+" . $arResult['REPEAT'] . " hours", 
                                            strtotime($LastTime)));
                            
                            if (strtotime($ReadyTime) >
    ...
    CSotbitReviews::SetCacheValues('SOTBIT_REVIEWS_QUESTIONS_ADD',$arResult);
    }
