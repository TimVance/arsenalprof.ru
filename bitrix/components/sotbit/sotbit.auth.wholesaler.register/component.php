...
use Sotbit\Auth\Internals\UserConfirmTable;
...
$arParams['LOGIN_EQ_EMAIL'] = Option::get("sotbit.auth", "LOGIN_EQ_EMAIL", "N", SITE_ID);
...
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["sotbit_auth_register"]) && !$USER->IsAuthorized())
{
	...
	if(isset($_REQUEST["REGISTER_WHOLESALER"]) && !empty($_REQUEST["REGISTER_WHOLESALER"]))
	{
		...
		$arResult["VALUES"]['WHOLESALER_FIELDS']['TYPE'] = $whosalerId;
		...
		$requredFields = unserialize(Option::get(SotbitAuth::idModule, 'GROUP_REQUIRED_FIELDS_' . $whosalerId, '', SITE_ID));
		...
		foreach ($fields as $key => $value)
		{
			...
			{
				$arResult["VALUES"]['WHOLESALER_FIELDS'][$key] = $value;
			...
			else
			{
				...
				$arResult["VALUES"]['WHOLESALER_FIELDS'][$key] = $_FILES["REGISTER_FILES_" . $key];
		...
		/*--- wholesaler ---*/
		$orderOptFields = unserialize(Option::get(SotbitAuth::idModule, 'GROUP_ORDER_FIELDS_' . $whosalerId, '', SITE_ID));
		...
		{
			$arResult["VALUES"]['WHOLESALER_ORDER_FIELDS'][$key] = $value;
		...
		if(isset($_REQUEST["REGISTER_WHOLESALER"]["TIME_ZONE"])) $arResult["VALUES"]['WHOLESALER_FIELDS']["TIME_ZONE"] = $_REQUEST["REGISTER_WHOLESALER"]["TIME_ZONE"];
	...
	if($arParams['LOGIN_EQ_EMAIL'] == 'Y')
        $arResult["VALUES"]['FIELDS']['LOGIN'] = $arResult["VALUES"]['WHOLESALER_FIELDS']['EMAIL'];
	else
	    $arResult["VALUES"]['FIELDS']['LOGIN'] = $_REQUEST["REGISTER"]['LOGIN'];
	...
	{
		$Values = array_merge($arResult["VALUES"]['FIELDS'], $arResult["VALUES"]['WHOLESALER_FIELDS']);
	}
	...
	else
	{
		...
		if($_REQUEST["TAB"] == 'WHOLESALER')
		{
			$Wholesaler = new WholeSaler();

			$Wholesaler->setPersonCurrentType($arResult["VALUES"]['WHOLESALER_FIELDS']['TYPE']);
			$Values['PERSON_TYPE'] = $arResult["VALUES"]['WHOLESALER_FIELDS']['TYPE'];
		...
		if($_REQUEST["TAB"] == 'WHOLESALER')
		{
			$Values['ORDER_FIELDS'] = $arResult["VALUES"]['WHOLESALER_ORDER_FIELDS'];
		}
...
{
	$APPLICATION->SetTitle(Loc::getMessage("REGISTER_DEFAULT_TITLE"));
}
