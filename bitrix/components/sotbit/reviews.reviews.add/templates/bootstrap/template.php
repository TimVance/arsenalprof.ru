				...
				if($arResult['BAN'] != "Y"):
					if((COption::GetOptionString(CSotbitReviews::iModuleID, "REVIEWS_IF_BUY_" . SITE_ID, "") == 'Y' && $arResult['REVIEWS_BUY'] == 'Y') || COption::GetOptionString(CSotbitReviews::iModuleID, "REVIEWS_IF_BUY_" . SITE_ID, "") != 'Y'):?>
						<? if($arResult['CAN_REPEAT'] == 1):?>
							<div class="row">
								<div class="col-sm-20 col-sm-offset-2 add-review-text">
								...
								<div class="col-sm-20 col-sm-offset-2 bg">
									...
									<form class="review" id="add_review" action="javascript:void(null);"
										...
										<input type="hidden" name="PRESENTATION_ERROR"
										       value="<?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_PRESENTATION_ERROR") ?>"/>
									...
									'TEXT' => array(
										...
										'NAME' => 'text'
												...
												<? else: ?>
													...
													<p class="count"><?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_ADD_COUNT") ?>
														<span class="count-now">0</span>
														/ <?= $arParams["TEXTBOX_MAXLENGTH"] ?></p>
										...
										<div class="radio">
											<input type="radio" name="RECOMMENDATED" value="Y" checked><span
													class="radio-label"><?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_RECOMMENDATED_YES") ?></span>
										</div>
										<div class="radio">
											<input type="radio" name="RECOMMENDATED" value="N"><span
													class="radio-label"><?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_RECOMMENDATED_NO") ?></span>
										</div>
										...
										<? if(COption::GetOptionString(CSotbitReviews::iModuleID, "REVIEWS_UPLOAD_IMAGE_" . SITE_ID, "") == 'Y'):?>
											<div class="add-photo">
												...
												<span id='add-photo-button'><i
															class="fa fa-plus"></i><?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_ADD_IMAGES") ?></span>
											</div>
											<ul id="preview-photo"
										...
										<? if($arResult['PRESENTATION_ALLOW'] == "Y"): ?>
											...
											<input type="text" name="presentation" value="" maxlength="255"
											       class="presentation"/>
										<? endif; ?>
										...
										<input type="submit" name="submit" id="review_submit"
										       value="<?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_SUBMIT_VALUE") ?>"/>
										<input type="button"
										       value="<?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_ADD_CANCEL") ?>"
										       id="reset-form">
									</form>
								</div>
							</div>
					...
					<? else:?>
						<p class="not-buy-error not-error"><?= GetMessage(CSotbitReviews::iModuleID . "_REVIEWS_IF_BUY_NOT_TITLE") ?></p>
					<? endif; ?>
				<? else:?>
